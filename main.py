...
!pip install PyPDF2 transformers torch sentence-transformers hdbscan wordcloud rake-nltk spacy
!python -m spacy download fr_core_news_sm  # Télécharger le modèle français pour spaCy
...
# Fonction pour filtrer les noms et les adjectifs avec spaCy
def filtrer_noms_et_adjectifs(texte):
    try:
        ...
        mots_filtres = [token.text for token in doc if token.pos_ in ['NOUN', 'ADJ']]
